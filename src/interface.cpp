                              ...
                              double           cycle_length)
{
    ...
    if (!ImGui::GetIO().WantCaptureMouse) {
        ...
        for (size_t i = 0; i < particles.positions.size(); ++i) {
            ...
            if (dist < min_dist_particle) { min_dist_particle = dist; closest_particle = (int)i; }
        }
        hover_particle_index = closest_particle;
        hover_organism_id = -1;
        if (closest_particle != -1) hover_organism_id = (int64_t)particles.stats[closest_particle].current_organism_id;
        else {
            ...
            for (const auto& org : org_manager.organisms) {
                ...
                if (dist < min_dist_org) { min_dist_org = dist; hover_organism_id = (int64_t)org.id; }
            }
        }
        if (hover_particle_index != -1 || hover_organism_id != -1) draw_hover_popup(particles, org_manager);
    }
}
...
void Interface::draw_hover_popup(const Particles& particles, const OrganismManager& org_manager) {
    ...
    if (hover_particle_index != -1) {
        ...
        ImGui::Separator();
        ImGui::Text("Type: %u | Age: %.1fs", type, stats.spawn_time);
        if (stats.current_organism_id != -1) ImGui::Text("Part of Organism #%d", stats.current_organism_id);
    }
    if (hover_organism_id != -1) {
        if (hover_particle_index != -1) ImGui::Separator();
        const Organism* org = nullptr;
        for (const auto& o : org_manager.organisms) if (o.id == (uint64_t)hover_organism_id) { org = &o; break; }
        if (org) {
            const glm::vec4& c = particles.colors[org->traits.dominant_type];
            ImGui::TextColored(ImVec4(c.r, c.g, c.b, 1.0f), "Organism #%lu", org->id);
            ImGui::Separator();
            ImGui::Text("Size: %u | Avg Speed: %.2f", org->traits.size, org->traits.avg_speed);
            ImGui::Text("Gen: %u | Kills: %u | Divs: %u", org->traits.generation, org->traits.kills, org->traits.divisions);
        }
    }
    ImGui::EndTooltip();
}
    ...
    if (ImGui::BeginTabBar("MetricsTabs")) {
        if (ImGui::BeginTabItem("Particles")) {
            if (ImGui::BeginTable("p_table", 6, ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_ScrollY, ImVec2(0, 400))) {
                ImGui::TableSetupColumn("ID"); ImGui::TableSetupColumn("Type"); ImGui::TableSetupColumn("Energy"); ImGui::TableSetupColumn("Age"); ImGui::TableSetupColumn("Convs"); ImGui::TableSetupColumn("Org");
                ImGui::TableHeadersRow();
                for (size_t i = 0; i < particles.positions.size(); ++i) {
                    ImGui::TableNextRow();
                    ImGui::TableSetColumnIndex(0); ImGui::Text("%zu", i);
                    ImGui::TableSetColumnIndex(1); ImGui::Text("%u", particles.types[i]);
                    ImGui::TableSetColumnIndex(2); ImGui::Text("%.2f", particles.energy[i]);
                    ImGui::TableSetColumnIndex(3); ImGui::Text("%.1f", particles.stats[i].spawn_time);
                    ImGui::TableSetColumnIndex(4); ImGui::Text("%u", particles.stats[i].conversion_count);
                    ImGui::TableSetColumnIndex(5); ImGui::Text("%d", particles.stats[i].current_organism_id);
                }
                ImGui::EndTable();
            }
            ImGui::EndTabItem();
        ...
        if (ImGui::BeginTabItem("Organisms")) {
            if (ImGui::BeginTable("o_table", 7, ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_ScrollY, ImVec2(0, 400))) {
                ImGui::TableSetupColumn("ID"); ImGui::TableSetupColumn("Type"); ImGui::TableSetupColumn("Size"); ImGui::TableSetupColumn("Speed"); ImGui::TableSetupColumn("Gen"); ImGui::TableSetupColumn("Kills"); ImGui::TableSetupColumn("Divs");
                ImGui::TableHeadersRow();
                for (const auto& org : org_manager.organisms) {
                    ImGui::TableNextRow();
                    ImGui::TableSetColumnIndex(0); ImGui::Text("%lu", org.id);
                    ImGui::TableSetColumnIndex(1); ImGui::Text("%u", org.traits.dominant_type);
    ...
    for (uint32_t row = 0; row <= pt; ++row) {
        for (uint32_t col = 0; col <= pt; ++col) {
            ...
            if (row == 0 && col == 0) ImGui::Dummy(ImVec2(cell_size, cell_size));
                ...
                glm::vec4& c = particles.colors[col-1];
                ImGui::ColorButton("##c", ImVec4(c.r, c.g, c.b, 1.0f), 0, ImVec2(cell_size, cell_size));
            } else if (col == 0) {
                glm::vec4& c = particles.colors[row-1];
                ImGui::ColorButton("##r", ImVec4(c.r, c.g, c.b, 1.0f), 0, ImVec2(cell_size, cell_size));
            } else {
                float& f = particles.forces[(col-1) + (row-1)*MAX_PARTICLE_TYPES];
                char label[16]; sprintf(label, "##f%u", (col-1) + (row-1)*MAX_PARTICLE_TYPES);
                ImGui::Button(label, ImVec2(cell_size, cell_size));
                if (ImGui::IsItemHovered()) f = std::clamp(f + ImGui::GetIO().MouseWheel * 0.1f, -1.0f, 1.0f);
            }
            ImGui::PopID();
